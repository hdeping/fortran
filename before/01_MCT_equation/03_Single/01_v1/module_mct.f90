...
subroutine getmatrixes()
    ...
    diffu = 1.0
    !  get K and R
    !  get A_q, gamma_q first
    do q = 1,ncut
        tmpa = dk(q)**2.0*diffu/sk(q)
        tmpb = 1.0 + sk(q)*v/diffu
        mat_R(q) = tmpa*tmpb
        mat_K(q) = tmpa/tmpb
    end do
    !  get matrix A
...
subroutine getmct2()
   ...
   do jj = 1,ncut
    do ii = 1,tmnum/2
        f(jj,ii)    = f(jj,ii*2 - 1)
     memory(jj,ii)  = memory(jj,ii*2 - 1)
     if(ii > 2)then
        par_f(jj,ii)   = f(jj,ii) - f(jj,ii - 1) 
    endif
    end do
   end do
...
function getfinal()
    ...
    do 
        ...
        do ii = 1,ncut
            ...
            tmpb(ii) = newmemory(ii)*mat_K(ii)*sk(ii)/ &
                       (mat_R(ii) + mat_K(ii)*newmemory(ii))
