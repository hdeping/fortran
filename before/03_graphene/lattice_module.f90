...
do ii = pos_graphene,pos_front+1
    if(ii>nx_ltc)exit    !pos_graphene+4 !pos_front+1                      !修改pos_front最大值为60，加1则数组越界
...
n_cfgT_D = 0
do ii = pos_graphene,pos_front+1
        if(ii>nx_ltc-1)exit        !修改于2014-9-9
    do jj = 1,ny_ltc                    ! 这两重循环为遍历生长区域内所有点
        do j_c = 1,6                    ! 这重循环为遍历以(ii,jj)点为起点的所有ci簇
            flag = 0
            do jj_c = 1,j_c            ! 这重循环遍历当前构型包含的所有位点
...
if(mod(i+j,2) = =0) then
    Neighbors(3,:) = (/i-1,j/)
    if(i = =1)Neighbors(3,:)=(/nx_ltc,j/)
else
    Neighbors(3,:) = (/i+1,j/)
    if(i = =nx_ltc)Neighbors(3,:)=(/1,j/)
endif
