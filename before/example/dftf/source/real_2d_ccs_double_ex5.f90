...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_DOUBLE_EX5
      ...
      real(8) :: X_IN_3D(M_MAX+2,N_MAX,K_MAX+2)
      real(8) :: X_IN   ((M_MAX+2)*N_MAX*(K_MAX+2))
      real(8) :: X_EXP  ((M_MAX+2)*N_MAX*(K_MAX+2))
      real(8) :: X_EXP_3D((M_MAX+2),N_MAX,(K_MAX+2))
...
!
      call ZERO_INIT_REAL_D(X_IN, (M_MAX+2)*N_MAX*(K_MAX+2))

      do i=1, multiple
        call INIT_MULTIPLE_2D_COLUMNS_D(X_IN_3D(1,i,1), m, n, strides_in)
      end do

      call DCOPY((M_MAX+2)*N_MAX*(K_MAX+2), X_IN, 1, X_EXP, 1)
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'INPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_D(X_IN_3D(1,i,1), lengths, strides_in)
          end do
      end if
