...
!
!       Forward-Backward 1D complex transform for single precision data
!
!*****************************************************************************
!
...
!
      ...
      if (LEGEND_PRINT) then
          print *
          print *, 'COMPLEX_1D_SINGLE_EX6'
          print *, 'Forward-Backward 1D complex transform for single precision data'
          print *
...
!
      ...
      call INIT_MULTIPLE_VECTOR_C(X_IN, n, multiple, dist_in, strides_in)
      call CCOPY(M_MAX*N_MAX, X_IN, 1, X_EXP, 1)

      if (ADVANCED_DATA_PRINT) then
        print *
        print *, 'INPUT vector X (for multiple=3)'
        call PRINT_THREE_VECTORS_C(X_IN, n, dist_in, strides_in)
      end if
...
!
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        call PRINT_THREE_VECTORS_C(X_OUT, n, dist_out, strides_out)
      end if
