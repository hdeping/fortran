...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_SINGLE_EX7
      ...
      real(4) :: X_IN_3D (M_MAX+2,N_MAX+2,K_MAX)
      real(4) :: X_IN    ((M_MAX+2)*(N_MAX+2)*K_MAX)
      real(4) :: X_EXP   ((M_MAX+2)*(N_MAX+2)*K_MAX)
      real(4) :: X_EXP_3D((M_MAX+2),(N_MAX+2),K_MAX)
...
!
      call ZERO_INIT_REAL_S(X_IN, (M_MAX+2)*(N_MAX+2)*K_MAX)

      do i=1, multiple
        call INIT_MULTIPLE_2D_COLUMNS_S(X_IN_3D(1,1,i), m, n, strides_in)
      end do

      call SCOPY((M_MAX+2)*(N_MAX+2)*K_MAX, X_IN, 1, X_EXP, 1)
