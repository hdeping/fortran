...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_DOUBLE_EX7
      ...
      real(8) :: X_IN_3D (M_MAX+2,N_MAX+2,K_MAX)
      real(8) :: X_IN    ((M_MAX+2)*(N_MAX+2)*K_MAX)
      real(8) :: X_EXP   ((M_MAX+2)*(N_MAX+2)*K_MAX)
      real(8) :: X_EXP_3D((M_MAX+2),(N_MAX+2),K_MAX)
...
!
      ...
      strides_in(2) = 1
      strides_in(3) = M_MAX+2

      dist_in = (M_MAX+2)*(N_MAX+2)

      if (LEGEND_PRINT) then
          print *
          print *, 'REAL_2D_CCS_DOUBLE_EX7'
...
!
      call ZERO_INIT_REAL_D(X_IN, (M_MAX+2)*(N_MAX+2)*K_MAX)

      do i=1, multiple
        call INIT_MULTIPLE_2D_COLUMNS_D(X_IN_3D(1,1,i), m, n, strides_in)
      end do

      call DCOPY((M_MAX+2)*(N_MAX+2)*K_MAX, X_IN, 1, X_EXP, 1)
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'INPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_D(X_IN_3D(1,1,i), lengths, strides_in)
          end do
      end if
...
!
      maxerr = 0.0
      do i=1, multiple
          err = CHECK_RESULT_2D_D(X_IN_3D(1,1,i), X_EXP_3D(1,1,i), lengths, strides_in)
