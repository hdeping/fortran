...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_SINGLE_EX8
      ...
      real(4) :: X_OUT_3D(M_MAX+2,N_MAX+2,K_MAX)
      real(4) :: X_OUT   ((M_MAX+2)*(N_MAX+2)*K_MAX)
...
!
      ...
      call ZERO_INIT_REAL_S(X_OUT,(M_MAX+2)*(N_MAX+2)*K_MAX)
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'Forward OUTPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_S(X_OUT_3D(1,1,i), lengths, strides_out)
          end do
      end if
