...
!*****************************************************************************
      ...
      Program REAL_2D_CCE_DOUBLE_EX4
      ...
      complex(8) :: X_OUT_3D(M_MAX,N_MAX/2+1,K_MAX)
      complex(8) :: X_OUT   (M_MAX*(N_MAX/2+1)*K_MAX)
...
!
      ...
      call ZERO_INIT_COMPLEX_Z(X_OUT, M_MAX*(N_MAX/2+1)*K_MAX)
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'Forward OUTPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_2D_COLUMNS_Z(X_OUT_3D(i,1,1), m, strides_out)
          end do
      end if
