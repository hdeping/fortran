...
!*****************************************************************************
      ...
      Program REAL_1D_CCS_DOUBLE_EX5
      ...
      real(8) :: X_IN_2D(M_MAX+2,N_MAX)
      real(8) :: X_IN   ((M_MAX+2)*N_MAX)
      real(8) :: X_EXP  ((M_MAX+2)*N_MAX)
...
!
      call ZERO_INIT_REAL_D(X_IN, (M_MAX+2)*N_MAX)
      call INIT_MULTIPLE_REAL_VECTOR_D(X_IN, n, multiple, dist_in, strides_in)
      call DCOPY((M_MAX+2)*N_MAX, X_IN, 1, X_EXP, 1)
...
!
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        call PRINT_THREE_VECTORS_D(X_IN, n+2, dist_in, strides_in)
      end if
...
!     Check result
!
      maxerr = CHECK_RESULT_D(X_IN, X_EXP,(M_MAX+2)*N_MAX)
