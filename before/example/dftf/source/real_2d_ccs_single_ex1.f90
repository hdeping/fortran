...
!           DFTI_INPUT_STRIDES  = {0, 1, M_MAX+2} (default={0,1,m})
...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_SINGLE_EX1
      ...
      real(4) :: X_IN_2D(M_MAX+2,N_MAX+2)
      real(4) :: X_IN   ((M_MAX+2)*(N_MAX+2))
      real(4) :: X_EXP  ((M_MAX+2)*(N_MAX+2))
...
!
      if ((m*n) .GT. (M_MAX+2)*(N_MAX+2)) then
...
!
      call ZERO_INIT_REAL_S(X_IN, (M_MAX+2)*(N_MAX+2))
      call INIT_MULTIPLE_REAL_VECTOR_S(X_IN, m, n, M_MAX+2, strides_in)
      call SCOPY((M_MAX+2)*(N_MAX+2), X_IN, 1, X_EXP, 1)

      if (ADVANCED_DATA_PRINT) then
        print *
        print *, 'INPUT vector X (2D columns)'
        call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_S(X_IN, lengths, strides_in)
      end if
