...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_DOUBLE_EX4
      ...
      real(8) :: X_OUT_3D(M_MAX,N_MAX+2,K_MAX+2)
      real(8) :: X_OUT   (M_MAX*(N_MAX+2)*(K_MAX+2))
...
!
      ...
      call ZERO_INIT_REAL_D(X_OUT, M_MAX*(N_MAX+2)*(K_MAX+2))
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'Forward OUTPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_D(X_OUT_3D(i,1,1), lengths, strides_out)
          end do
      end if
