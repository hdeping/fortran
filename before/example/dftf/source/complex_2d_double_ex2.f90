...
!
      ...
      call INIT_MULTIPLE_VECTOR_Z(X_IN, m, n, M_MAX, strides_in)
      call ZCOPY(M_MAX*N_MAX, X_IN, 1, X_EXP, 1)

      if (ADVANCED_DATA_PRINT) then
        print *
        print *, 'INPUT vector X (2D columns)'
        call PRINT_THREE_VECTORS_Z(X_IN, m, M_MAX, strides_in)
      end if
...
!
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        call PRINT_THREE_VECTORS_Z(X_OUT, m, M_MAX, strides_out)
      end if
