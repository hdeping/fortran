...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_DOUBLE_EX1
      ...
      real(8) :: X_IN_2D(M_MAX+2,N_MAX+2)
      real(8) :: X_IN   ((M_MAX+2)*(N_MAX+2))
      real(8) :: X_EXP  ((M_MAX+2)*(N_MAX+2))
...
!
      call ZERO_INIT_REAL_D(X_IN, (M_MAX+2)*(N_MAX+2))
      call INIT_MULTIPLE_REAL_VECTOR_D(X_IN, m, n, M_MAX+2, strides_in)
      call DCOPY((M_MAX+2)*(N_MAX+2), X_IN, 1, X_EXP, 1)

      if (ADVANCED_DATA_PRINT) then
        print *
        print *, 'INPUT vector X (2D columns)'
        call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_D(X_IN, lengths, strides_in)
      end if
