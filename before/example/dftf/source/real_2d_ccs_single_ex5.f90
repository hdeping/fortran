...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_SINGLE_EX5
      ...
      real(4) :: X_IN_3D(M_MAX+2,N_MAX,K_MAX+2)
      real(4) :: X_IN   ((M_MAX+2)*N_MAX*(K_MAX+2))
      real(4) :: X_EXP  ((M_MAX+2)*N_MAX*(K_MAX+2))
      real(4) :: X_EXP_3D((M_MAX+2),N_MAX,(K_MAX+2))
...
!
      ...
      strides_in(2) = 1
      strides_in(3) = (M_MAX+2)*N_MAX

      dist_in = M_MAX+2

      if (LEGEND_PRINT) then
          print *
          print *, 'REAL_2D_CCS_SINGLE_EX5'
...
!
      call ZERO_INIT_REAL_S(X_IN, (M_MAX+2)*N_MAX*(K_MAX+2))

      do i=1, multiple
        call INIT_MULTIPLE_2D_COLUMNS_S(X_IN_3D(1,i,1), m, n, strides_in)
      end do

      call SCOPY((M_MAX+2)*N_MAX*(K_MAX+2), X_IN, 1, X_EXP, 1)
      ...
      if (ADVANCED_DATA_PRINT) then
        ...
        print *, 'INPUT vector X (2D columns)'
          do i=1, multiple
              print *
              print 908, i
              call PRINT_THREE_FIRST_AND_THREE_LAST_VECTORS_2D_S(X_IN_3D(1,i,1), lengths, strides_in)
          end do
      end if
...
!
      maxerr = 0.0
      do i=1, multiple
          err = CHECK_RESULT_2D_S(X_IN_3D(1,i,1), X_EXP_3D(1,i,1), lengths, strides_in)
