...
!*****************************************************************************
      ...
      Program REAL_2D_CCS_SINGLE_EX3
      ...
      real(4) :: X_IN_3D(M_MAX,N_MAX+2,K_MAX+2)
      real(4) :: X_IN   (M_MAX*(N_MAX+2)*(K_MAX+2))
      real(4) :: X_EXP  (M_MAX*(N_MAX+2)*(K_MAX+2))
...
!
      call ZERO_INIT_REAL_S(X_IN, M_MAX*(N_MAX+2)*(K_MAX+2))

      do i=1, multiple
        call INIT_MULTIPLE_2D_COLUMNS_S(X_IN_3D(i,1,1), m, n, strides_in)
      end do

      call SCOPY(M_MAX*(N_MAX+2)*(K_MAX+2), X_IN, 1, X_EXP, 1)
