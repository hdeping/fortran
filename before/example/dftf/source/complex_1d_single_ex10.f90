...
!
!       Backward 1D complex transform for single precision data not inplace
...
!*****************************************************************************
      ...
      Program COMPLEX_1D_SINGLE_EX10
...
!
      ...
      Status = DftiComputeBackward(hand, X_IN, X_OUT)
      if (.NOT. DftiErrorClass(Status, DFTI_NO_ERROR) ) then
          call dfti_example_status_print(Status)
          print *, 'TEST FAILED : DftiComputeBackward(hand, X_IN, X_OUT)'
          failure = .TRUE.
          goto 100
      end if
...
!     Check result
!
      maxerr = CHECK_RESULT_C(X_OUT, X_EXP, n)
